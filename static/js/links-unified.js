// Unified Links Page JavaScript
(function() {
    'use strict';
    
    // URL mappings for social platforms
    const urlMappings = {
    ...
    // Create hover menu with social icons
    function createHoverMenu(urls) {
        const menu = document.createElement('div');
        menu.className = 'hover-menu';
        
        // Instagram icon (always present)
        const instagramIcon = createSocialIcon('instagram', urls.instagram);
        menu.appendChild(instagramIcon);
        
        // Website icon (if URL exists)
        if (urls.website) {
            const websiteIcon = createSocialIcon('website', urls.website);
            menu.appendChild(websiteIcon);
        } else {
            const disabledWebsite = createSocialIcon('website', null);
            disabledWebsite.classList.add('disabled');
            menu.appendChild(disabledWebsite);
        }
        
        // YouTube icon (if URL exists)
        if (urls.youtube) {
            const youtubeIcon = createSocialIcon('youtube', urls.youtube);
            menu.appendChild(youtubeIcon);
        } else {
            const disabledYoutube = createSocialIcon('youtube', null);
            disabledYoutube.classList.add('disabled');
            menu.appendChild(disabledYoutube);
        }
    ...
    function createSocialIcon(platform, url) {
        ...
        // Add SVG icon
        link.innerHTML = getSvgIcon(platform);
    ...
    // Process all links and add hover menus
    function processLinks() {
        // Find all Instagram links (they have the link-item-wrapper class directly on them)
        const links = document.querySelectorAll('.link-grid a.link-item-wrapper[href*="instagram.com"]');
        
        links.forEach(link => {
            // Skip if already processed (has hover menu)
            if (link.querySelector('.hover-menu')) return;
            ...
            // Since the link already has link-item-wrapper class, just add the hover menu
    ...
    function initCollapsibles() {
        const headers = document.querySelectorAll('.instagram-links h3, .instagram-links h4');
